...
class CoinInteractor: CoinBusinessLogic, CoinDataStore {
    ...
    func doFetchMarketChart(request: Coin.FetchMarketChart.Request) {
        ...
        marketChartDataWorker?.doFetchMarketChart(id: coinId,
                                                  baseCoin: baseCoin,
                                                  from: from,
                                                  to: to,
    ...
    func doFetchOhlc(request: Coin.FetchOhlc.Request) {
        ...
        ohlcWorker?.doFetchMarketChart(id: coinId,
                                       baseCoin: baseCoin,
                                       of: day,
                                       completion: { result in
            switch result {
    ...
    private func createCurrentDataResponse(baseCoin: CoinsFilterEnum, currentData: CurrentDataModel?) {
        if let currentData {
            let response = Coin.FetchCurrentData.Response(baseCoin: baseCoin,
                                                          ...
                                                          maximumValuePercentege: currentData.marketData.athChangePercentage[baseCoin.rawValue] ?? 0.0,
                                                          ...
                                                          minimumValuePercentege: currentData.marketData.atlChangePercentage[baseCoin.rawValue] ?? 0.0,
    ...
    private func createMarketChartResponse(marketChart: MarketChartModel?) {
        if let marketChart {
            let response = marketChart.prices.map { prices in
                return Coin.FetchMarketChart.Response(price: prices[1],
    ...
    private func createOhlcResponse(graphicData: [GraphicDataModel]?) {
        if let graphicData {
            let response = graphicData.map { data in
                return Coin.FetchMarketChart.Response(price: data[4],
